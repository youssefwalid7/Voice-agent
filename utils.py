...
#from vector import PineconeManager   
from langchain_openai import ChatOpenAI
from langchain.prompts import PromptTemplate
import logging
load_dotenv()
def save_text_to_file(text, filename="output.txt", mode="w"):
    """
    Saves the given text to a file.

    :param text: The text to save.
    :param filename: The name of the file (default is "output.txt").
    :param mode: The file mode ('w' for overwrite, 'a' for append).
    """
    try:
...
def retrieve_url_headers(url):
    try:
        ...
        # Find all headers from h1 to h6
        headers = []
        for i in range(3,4):  # h1 to h6
            headers += [header.get_text(strip=True) for header in soup.find_all(f'h{i}')]
...
def read_file_lines_to_list(file_path):
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            lines = [line.strip() for line in lines]  # Optional: strip leading/trailing whitespace
            
        return lines
    except Exception as e:
        print(f"Error reading file: {e}")
        return None
def retrieve_url_all_content(url):
    try:
        response = requests.get(url, timeout=5)
        response.raise_for_status()
        ...
        # Find all headers from h1 to h6 and their content
        content = []
        for i in range(1, 7):  # h1 to h6
            headers = soup.find_all(f'h{i}')
            for header in headers:
                header_text = header.get_text(strip=True)
                # Get the content after each header (until the next header)
                content_after_header = []
                sibling = header.find_next_sibling()
                while sibling and sibling.name not in [f'h{j}' for j in range(1, 7)]:  # Stop at the next header
                    if sibling.name == 'p':  # Only include paragraph content
                        content_after_header.append(sibling.get_text(strip=True))
                    sibling = sibling.find_next_sibling()
                
                # Store the header and its content in the content list
                content.append({'header': header_text, 'content': " ".join(content_after_header)})
        
        return content  # Return the list of headers with their content
    
    except Exception as e:
        print(f"Error retrieving headers and content: {e}")
        return None  # Return None if content retrieval fails
...
def read_json(file_path):
    ...
    :return: Dictionary containing JSON data or None if an error occurs.
    """
    try:
        with open(file_path, "r", encoding="utf-8") as file:
...
def save_json(data, file_path):
    ...
    try:
        with open(file_path, "w", encoding="utf-8") as file:
            json.dump(data, file, indent=4, ensure_ascii=False)  # Pretty format
        print(f"JSON data saved successfully to {file_path}")
    except Exception as e:
        print(f"Error saving JSON file: {e}")
def read_json_files_in_folder(folder_path):
    json_data = {}  # Dictionary to store the contents of all JSON files
    
    # Loop through files in the directory
    for filename in os.listdir(folder_path):
        # Check if the file ends with .json (JSON file)
        if filename.endswith('.json'):
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                # Store the content of each JSON file in the dictionary
                json_data[filename] = data
    
    # Return the dictionary containing all JSON data
    return json_data


def dict_list_to_readable_string(data):
    """
    Converts a list of dictionaries into a readable string format.
    ...
    for idx, item in enumerate(data, start=1):
        readable_string += f"Item {idx}:\n"
        for key, value in item.items():
            readable_string += f"  {key}: {value}\n"
